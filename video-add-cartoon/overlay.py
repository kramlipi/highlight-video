...
from paths import DEFAULT_CARTOON, DEFAULT_OUTPUT_DIR
...
def fps_to_float(fps: str) -> float:
    try:
...
def build_filter_complex(main: VideoInfo, overlay_w: int, overlay_h: int) -> str:
    ...
    # Cartoon source is green-screen vertical video: chromakey first, then oval alpha.
    oval_alpha = (
        "if(lte(pow((X-W/2)/(W/2),2)+pow((Y-H/2)/(H/2),2),1),alpha(X,Y),0)"
    )
    return (
        f"[1:v]scale={overlay_w}:{overlay_h}:flags=lanczos,format=rgba,"
        f"chromakey={CHROMAKEY_COLOR}:{CHROMAKEY_SIMILARITY}:{CHROMAKEY_BLEND},"
        f"geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='{oval_alpha}'[fg];"
        f"[0:v]setsar=1,setpts=PTS-STARTPTS[base];"
        f"[base][fg]overlay={x}:{y}:shortest=1:format=auto[outv]"
    )
...
def default_final_output_path(input_path: Path) -> Path:
    return DEFAULT_OUTPUT_DIR / f"{input_path.stem}_final{input_path.suffix}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output video path (default: /mnt/d/karm/video_tutorial/<stem>_final.ext)",
    )
    parser.add_argument(
        "--cartoon",
        type=Path,
    ...
    output = args.output or default_final_output_path(args.input)
